...
async function fetchKurser(userClass) {
    try {
        ...
        kurserState.value = res.data.map(kurs => {
            ...
            return { ...kurs, date, time, teacher: kurs.teacher || '', description: kurs.description || '' }
        })
    } catch (err) {
...
async function addKurs() {
    ...
    try {
        const res = await api.post('/items',
            { name: newName.value, date: localDateTime, teacher: newTeacher.value, description: newDescription.value, class: newClass.value }
        )
...
async function updateKurs(kurs) {
    try {
        const localDateTime = `${kurs.date}T${kurs.time}`
        const res = await api.put(`/items/${kurs._id}`,
            { name: kurs.name, date: localDateTime, teacher: kurs.teacher, description: kurs.description, class: kurs.class }
        )
